...
spec_hash =
  describe "placeholder" $ do
    it "output called 'object'" $ do
      ...
      shouldBe (renderPlaceholder $ createPlaceholder (OutputName $ unsafeName "object"))
               objectPlaceholder
    it "output called 'interface'" $ do
      let interfacePlaceholder = "/1ang7n5l91vn079693l42ahmcxgf34r0qad1l01y4lf7d3cwm5lg"
      shouldBe (renderPlaceholder $ createPlaceholder (OutputName $ unsafeName "interface"))
               interfacePlaceholder

-- | Unsafely create a StorePathName, erroring on invalid input.
-- Only use in tests with known-good literals.
unsafeName :: Text -> StorePathName
unsafeName = either (error . show) id . mkStorePathName

spec_nixhash :: Spec
spec_nixhash =
  describe "downstream placeholder" $ do
    it "known derivation, unknown output store path (CA)" $ do
      Right p <- pure $ parsePathFromText (StoreDir "/nix/store") "/nix/store/g1w7hy3qg1w7hy3qg1w7hy3qg1w7hy3q-foo.drv"
      let dp = renderDownstreamPlaceholder $ unknownCaOutput p (OutputName $ unsafeName "out")
    ...
    it "unknown derivation" $ do
      ...
      let dp = renderDownstreamPlaceholder $ unknownDerivation (unknownCaOutput p (OutputName $ unsafeName "out")) (OutputName $ unsafeName "out")
